...
class D2Prune_QWEN:
    ...
    def prepare_layer_calibration(self, train_loader, layer_ind=0):
        ...
        class Catcher(nn.Module):
            def __init__(self, module, cache, inps):
                super().__init__()
                self.module = module
                self.cache = cache
                self.inps = inps
            ...
            def forward(self, inp, **kwargs):
                # inp shape usually [1, seq_len, hidden_size]
                self.inps[self.cache['i']].copy_(inp[0])
                self.cache['i'] += 1
                self.cache['attention_mask'] = kwargs.get('attention_mask', None)
                self.cache['position_ids'] = kwargs.get('position_ids', None)
                self.cache['position_embeddings'] = kwargs.get('position_embeddings', None)
                raise ValueError
        # self.layers[layer_ind] = Catcher(self.layers[layer_ind])
        self.layers[layer_ind] = Catcher(self.layers[layer_ind], cache, inps)

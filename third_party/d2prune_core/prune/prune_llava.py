...
class D2Prune_LLAVA:
    ...
    def prune_llm(self, train_loader):
        ...
        for i in tqdm(range(len(self.layers)), desc='Pruning Processing'):
            ...
            if f"model.layers.{i}" in self.hf_device_map:  # multiple gpu can run, this means model init by "auto"
                dev = self.hf_device_map[f"model.layers.{i}"]
            ...
            if self.args.dsm == 'owl':
                ...
                if self.save_activations:
                    self.layers_activations.append((torch.norm(inps, p=2, dim=(0,1)) ** 1 /inps.shape[0]).cpu().numpy().tolist()) # 2C
    ...
    def prune_llm(self, train_loader):
        ...
        for i in trange(len(self.layers), desc='Pruning Processing'):
            ...
            if self.args.dsm == 'owl':
                ...
                if self.save_attention_score: # seqlen=128
                    sparse_attn = self.capture_attention_output(layer, inps, attention_mask, position_ids, position_embeddings) # [32,128,128] # numpy
                    self.layers_attention_score.append(sparse_attn)
                # 3. forward layers
                for j in range(self.nsamples):
                ...
                inps, outs = outs, inps
                if self.save_activations:
                    self.layers_activations.append((torch.norm(inps, p=2, dim=(0,1)) ** 1 /inps.shape[0]).cpu().numpy().tolist()) # not 2， to void inf value
                del layer, subset, gpts
                gc.collect()
                torch.cuda.empty_cache()
                if self.args.free:

...
class D2Prune_MISTRAL:
    ...
    def capture_attention_output(self, layer, inps, attention_mask, position_ids, position_embeddings):
        """
        ...
        with torch.no_grad():
            # MISTRAL Layer forward 返回值通常是 (hidden_states, self_attn_weights, present_key_value)
    ...
    def check_sparsity(self, tolerance=1e-6):
        ...
        error = abs(float(count) / total_params - self.args.sparsity_ratio)
        if error <= tolerance:
    ...
    def mezo_dsm(self, global_sparsity, inps, outs, attention_mask, position_ids, position_embeddings, layer,
        ...
        with torch.no_grad():
            ...
            # p = float(global_sparsity) if not isinstance(global_sparsity, list) else float(self.args.sparsity_ratio)
            # c = torch.tensor(p, device=device, dtype=dtype) * N.sum()
            # r = self._project_C_hyperplane_box(r, N, c, r_min_local, r_max_local)
        ...
        # 数值不溢出
        dense_Y = torch.zeros((inps.shape[0], inps.shape[1], inps.shape[2]),
                            device=inps.device, dtype=torch.float32)
        for j in range(inps.shape[0]):
            with torch.no_grad(), torch.cuda.amp.autocast(False):
                y = layer(inps[j].unsqueeze(0).float(),
                        attention_mask=attention_mask,
                        position_ids=position_ids,
                        position_embeddings=position_embeddings)[0]
                dense_Y[j] = y.squeeze(0).float()
        ...
        def prune_and_loss(current_r, X, Y, seed_for_all):
            ...
            # 2) 用当前 batch 的 X/Y 在 layer_copy 上做 forward_wrapper 收集统计量
            # inps_eval = X.detach()
            # outs_eval = torch.zeros_like(inps_eval)

            if self.args.use_wanda_forward:
                subset_copy, gpts_copy, wrapped_layers_copy = self.forward_layer_wrapper1(
                    layer_copy, X, Y, attention_mask, position_ids, position_embeddings
                )
            else:
                subset_copy, gpts_copy, wrapped_layers_copy = self.forward_layer_wrapper(
                    layer_copy, X, Y, attention_mask, position_ids, position_embeddings
                )
            ...
            with torch.no_grad():
                ...
                if not torch.isfinite(Y).all():
                    self.logger.info(f"[MEZO-DIAG] Y has non-finite at layer {layer_idx}")
                if not torch.isfinite(sparse_Y).all():
                    self.logger.info(f"[MEZO-DIAG] sparse_Y has non-finite at layer {layer_idx}")
                # 若输出存在非有限值，返回极大损失
                if (not torch.isfinite(sparse_Y).all()) or (not torch.isfinite(Y).all()):
                    ...
                    # 关键：float32 计算，避免 fp16 overflow
                    sparse_f = sparse_Y.float()
                    Y_f = Y.float()
                    ...
                    mse = (diff * diff).mean()   # float32
                    den = Y_f.var(unbiased=False)
                    den = torch.clamp(den, min=1e-6)

                    loss = mse / den
                # 关键：把 inf/nan 直接变成大数，避免污染 SPSA
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                for _ in range(n_spsa_epoch):
                    ...
                    if isinstance(global_sparsity, list):
                        c = torch.tensor(self.args.sparsity_ratio, device=device, dtype=dtype) * N.sum()
                        ...
                        # r_plus = self._project_C_hyperplane_box(r + eps_r*z, N, c, r_min_local, r_max_local)
                        # r_minus = self._project_C_hyperplane_box(r - eps_r*z, N, c, r_min_local, r_max_local)
                    else:
                        c = torch.tensor(global_sparsity, device=device, dtype=dtype) * N.sum()
                        # r_plus = self._project_C_hyperplane_box(r + eps_r*z, N, c, r_min_local, r_max_local)
                        # r_minus = self._project_C_hyperplane_box(r - eps_r*z, N, c, r_min_local, r_max_local)
        ...
        for i in tqdm(range(len(self.layers)), desc='Pruning Processing'):
            ...
            if self.args.dsm == 'owl':
                ...
                if self.save_attention_score:
                    sparse_attn = self.capture_attention_output(layer, inps, attention_mask, position_ids, position_embeddings) # [32,128,128] # numpy
